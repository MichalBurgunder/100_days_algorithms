...
function algorithm_x(adjecency_matrix, excluded_rows=[]) 
    ...
    for i in 1:size(adjecency_matrix)[1]
        if !(i in excluded_rows) && sums_of_ones[i] == minimum_number
            push!(rows, i)
        end
    end
    ...
    while(true)
        ...
        rows = filter!(element -> element != row_to_choose, a)
        
        rows_to_exclude = []
        for i in 1:size(adjecency_matrix)[1]
            if sums_of_ones[row_to_choose,i] == 1
                for j in 1:size(adjecency_matrix)[1]
                    if i != j && adjecency_matrix[i,j] == 1
        ...
        all_excluded_rows = collect(set(vcat(excluded_rows, rows_to_exclude)))
        if size(all_excluded_rows)[1] == size(adjecency_matrix)[1]
