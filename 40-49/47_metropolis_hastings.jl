...
function plot_results(og_data, syn_data)
    ...
    # samples = metropolis_hastings_simulation(10000)
    display(histogram(syn_data, bins=100)) # label="MH Samples"
    sleep(3)
    display(histogram(og_data, bins=100)) # xlabel="Value", ylabel="Frequency", title="Histogram of some Data"
    # display(plot!(x_vals, y_vals, label="True Distribution", linewidth=2))
    sleep(3)

end

function sample_from_target_dist(number_samples)
    final_samples = zeros(number_samples, 1)
    for i in 1:number_samples
        final_samples[i] = target_probability(rand())
    end
...
function metropolis_hastings_simulation(data, number_samples)
    ...
    synthetic_data = zeros(number_samples, 1)

    target_distribution = get_frequency_counts(data)

    # println(target_distribution)
    # exit()
    for i in 1:number_samples
        ...
        synthetic_data[i] = current_sample
...
synthetic_data = metropolis_hastings_simulation(original_data, 1000)
